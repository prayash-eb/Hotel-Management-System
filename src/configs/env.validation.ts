export function validateEnvironment() {
  ...
  if (
    process.env.JWT_ACCESS_TOKEN_EXPIRY_MS &&
    isNaN(parseInt(process.env.JWT_ACCESS_TOKEN_EXPIRY_MS))
  ) {
    invalidVars.push('JWT_ACCESS_TOKEN_EXPIRY_MS (must be a number)');
  }

  if (
    process.env.JWT_REFRESH_TOKEN_EXPIRY_MS &&
    isNaN(parseInt(process.env.JWT_REFRESH_TOKEN_EXPIRY_MS))
  ) {
